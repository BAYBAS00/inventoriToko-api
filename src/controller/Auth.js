...
const register = async (req, res) => {
    try {
        ...
        await authModel.registerUser({ username, email: emailInsensitive, password: hashedPassword, createdAt, updatedAt });
...
const login = async (req, res) => {
    try {
        ...
        // const token = jwt.sign({ name: user.username }, 'secretkey', { expiresIn: '365d' });
        const token = jwt.sign({ id: user.id, name: user.username }, 'secretkey', { expiresIn: '365d' });
        res.json({
            error: false,
